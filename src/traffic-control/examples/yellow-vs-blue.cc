...
int main (int argc, char *argv[])
{
  ...
  std::string queueDiscType = "YELLOW";
  uint16_t port = 5001;
  ...
  cmd.AddValue ("queueDiscType", "Set Queue disc type to YELLOW or BLUE", queueDiscType);
  ...
  if ((queueDiscType != "BLUE") && (queueDiscType != "YELLOW"))
    {
      std::cout << "Invalid queue disc type: Use --queueDiscType=YELLOW or --queueDiscType=BLUE" << std::endl;
      exit (1);
    }
  ...
  if (!modeBytes)
    {
      ...
      Config::SetDefault ("ns3::YellowQueueDisc::Mode", StringValue ("QUEUE_MODE_PACKETS"));
      Config::SetDefault ("ns3::YellowQueueDisc::QueueLimit", UintegerValue (queueDiscLimitPackets));
    }
  else
    {
  ...
  if (queueDiscType == "YELLOW")
    {
      Config::SetDefault ("ns3::YellowQueueDisc::Alpha", DoubleValue (1.02));
      Config::SetDefault ("ns3::YellowQueueDisc::Beta", DoubleValue (1.0));
      Config::SetDefault ("ns3::YellowQueueDisc::Delta", DoubleValue (0.06));
  ...
  if (queueDiscType == "YELLOW")
    {
      tchBottleneck.SetRootQueueDisc ("ns3::YellowQueueDisc");
    }
  else
    {
